...
│  Disk 0      │  NVMe — System (/, /boot)     │
│  Disk 1      │  NVMe — Data (/home)          │
├──────────────┴───────────────────────────────┤
│              Software Stack                   │
├──────────────┬───────────────────────────────┤
│  Kernel      │  linux-zen                    │
...
```
nvme0n1 (System)              nvme1n1 (Data)
├── p1: EFI (1G, FAT32)      └── p1: Btrfs
└── p2: Btrfs                      └── @home → /home
    ├── @          → /
    ├── @snapshots → /.snapshots
    ├── @log       → /var/log
...
```jsonc
{
  ...
  "disk": { "root": "/dev/nvme0n1", "home": "/dev/nvme1n1", "efi_size": "1G" },
  "hostname": "arch",
...
post-install 会安装：yay → XMind / 百度网盘 / 微信 → KDE Connect 防火墙规则 → HyDE 桌面
